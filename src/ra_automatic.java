...
class RandBoard
{
	...
	{
		num_colors = 0;
		RandBoard test_board = new RandBoard(this);
		test_board.process();
...
public class ra_automatic extends Canvas
{
	static Scanner sc = new Scanner(System.in);
	static RandBoard[] list;
	static int[] prev;
	static int next;
	...
	static String path;
	static void add_to_queue(RandBoard B, int prev_index)
	{
		int encode = B.encode();
	...
	static RandBoard current_board = new RandBoard();
	static void run()
	{
		String excluded = "6";
		...
		for(int i = 0; i < 5; i++)
			for(int j = 0; j < 6; j++)
			{
				RandBoard B = new RandBoard(current_board);
				B.cur_r = i;
				B.cur_c = j;
				add_to_queue(B, -1);
			}
		...
		for(int iter = 0; iter < num_iterations; iter++)
		{
			...
			{
				RandBoard B = new RandBoard(list[board_index]);
				B.move(-1, 0);
	...
	{
		ra_automatic graphics = new ra_automatic();
	...
	void display(Graphics G, RandBoard B)
	{
		for(int r = 0; r < 5; r++)
			for(int c = 0; c < 6; c++)
			{
	...
	public void paint(Graphics G)
	{
		while(true)
		{
			...
			current_board = new RandBoard(B);
			current_board.num_colors = 0;
