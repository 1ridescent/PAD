...
class RandBoardTricolor
{
	...
	RandBoardTricolor()
	{
		do
		{
			for(int i = 0; i < 30; i++) board[i] = rand_tricolor();
			num_combos = 0;
		}
		while(find_matches());
	}
	RandBoardTricolor(RandBoardTricolor copy)
	{
	...
	{
		num_combos = 0;
		RandBoardTricolor test_board = new RandBoardTricolor(this);
		test_board.process();
...
public class combo_tester_tricolor extends Canvas
{
	static Scanner sc = new Scanner(System.in);
	static RandBoardTricolor[] list;
	static int[] prev;
	static int next;
	...
	static String path;
	static void add_to_queue(RandBoardTricolor B, int prev_index)
	{
	...
	static RandBoardTricolor current_board = new RandBoardTricolor();
	...
	static void run()
	{
		...
		list = new RandBoardTricolor[MAX];
		prev = new int[MAX];
		next = 0;
		...
		for(int i = 0; i < 5; i++)
			for(int j = 0; j < 6; j++)
			{
				RandBoardTricolor B = new RandBoardTricolor(current_board);
				B.cur_r = i;
				B.cur_c = j;
				add_to_queue(B, -1);
			}
		...
		for(int iter = 0; iter < num_iterations; iter++)
		{
			...
			{
				RandBoardTricolor B = new RandBoardTricolor(list[board_index]);
				B.move(-1, 0);
	...
	{
		combo_tester_tricolor graphics = new combo_tester_tricolor();
	...
	void display(Graphics G, RandBoardTricolor B)
	{
		for(int r = 0; r < 5; r++)
			for(int c = 0; c < 6; c++)
			{
	...
	public void paint(Graphics G)
	{
		while(true)
		{
			...
			current_board = new RandBoardTricolor(B);
